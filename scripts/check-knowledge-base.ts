...
const samples: Record<string, Answers> = {
  ...
  "arriendo-comercial": {
    contrato_ciudad: "Santiago",
    contrato_fecha: "2026-08-01",
    inmueble_subtipo: "local_comercial",
    en_centro_comercial: false,
    arrendador_naturaleza: "juridica",
    arrendador_razon_social: "Rentas Urbanas SpA",
    arrendador_tipo_societario: "sociedad por acciones",
    arrendador_rep_nombre: "Marta Vidal",
    arrendador_rep_rut: "12.345.678-5",
    arrendador_rut: "76.123.456-0",
    arrendador_domicilio: "Av. Apoquindo 3000, of 500",
    arrendador_comuna: "Las Condes",
    arrendador_email: "arriendos@rentasurbanas.cl",
    titulo_calidad: "dueno",
    titulo_fojas: "8901",
    titulo_numero: "4567",
    titulo_anio: "2018",
    titulo_conservador: "Santiago",
    arrendatario_naturaleza: "juridica",
    arrendatario_razon_social: "Café del Barrio Ltda",
    arrendatario_tipo_societario: "sociedad de responsabilidad limitada",
    arrendatario_rep_nombre: "José Pinto",
    arrendatario_rep_rut: "9.876.543-3",
    arrendatario_rut: "77.777.777-7",
    arrendatario_domicilio: "Calle Bandera 150",
    arrendatario_comuna: "Santiago",
    arrendatario_email: "jose@cafedelbarrio.cl",
    inmueble_direccion: "Av. Providencia 2100",
    inmueble_numero_unidad: "local 3",
    inmueble_comuna: "Providencia",
    inmueble_region: "Región Metropolitana",
    inmueble_rol_sii: "555-12",
    inmueble_superficie: 85,
    inmueble_tiene_instalaciones: true,
    inmueble_amoblado: false,
    giro_descripcion: "cafetería y venta de pastelería",
    giro_codigo_sii: "561000",
    giro_sustancias_peligrosas: false,
    permisos_variante: "neutra",
    permisos_plazo_condicion: 60,
    plazo_modalidad: "renovable",
    plazo_inicio: "2026-09-01",
    plazo_meses: 36,
    plazo_termino: "2029-08-31",
    plazo_aviso: 60,
    plazo_opcion_renovacion: true,
    plazo_periodos_opcion: 1,
    plazo_meses_opcion: 36,
    plazo_aviso_opcion: 90,
    habilitacion_aplica: true,
    habilitacion_variante: "neutra",
    habilitacion_plazo_dias: 45,
    habilitacion_gracia_dias: 30,
    habilitacion_meses_amortizacion: 24,
    renta_modalidad: "fija",
    renta_moneda: "CLP",
    renta_monto_clp: 1800000,
    renta_dia_pago: 5,
    renta_reajuste_frecuencia: "anualmente",
    caucion_tipo: "deposito",
    caucion_rentas: 2,
    caucion_dias_restitucion: 30,
    fianza_matriz_aplica: false,
    seguros_rc_uf: 2000,
    seguros_exige_endoso: true,
    seguros_dias_acreditacion: 15,
    operacion_horario: "de funcionamiento del sector",
    operacion_dias_cierre_max: 3,
    operacion_pena_diaria: 2,
    exclusividad_aplica: false,
    cesion_variante: "neutra",
    cesion_dias_aviso: 15,
    cesion_dias_termino: 90,
    terminacion_variante: "neutra",
    terminacion_aviso_dias: 90,
    terminacion_multa_rentas: 3,
    terminacion_plazo_subsanacion: 30,
    restitucion_dias_retiro: 10,
    restitucion_recargo_pct: 50,
...
for (const contract of contractTypes) {
  ...
  if (contract.id === "arriendo-comercial") {
    const full = clauses.map((c) => `${c.heading}\n${c.text}`).join("\n");
    check(full.includes("Impuesto al Valor Agregado"), "IVA declarado (inmueble con instalaciones)");
    check(ids.has("operacion"), "cláusula de operación continua (local comercial)");
    check(!ids.has("exclusividad"), "exclusividad OMITIDA (no aplica)");
    check(ids.has("habilitacion"), "cláusula de habilitación incluida");
    check(full.includes("$1.800.000"), "renta CLP formateada");
  }
